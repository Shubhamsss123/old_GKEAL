...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if (not args.incremental and not args.multiprocessing_distributed) or (args.multiprocessing_distributed
                                                and args.rank % ngpus_per_node == 0):
        os.rename(args.dirname, args.dirname + '_' + str(round(acc1.item(),2)) + '-' + str(round(best_acc1.item(),2)))
        print('Renaming fold to: '+ args.dirname + '_' + str(round(acc1.item(),2)) + '-' + str(round(best_acc1.item(),2)))

    if args.incremental:
        bias_fe = False
        # get expansion weight
        if hasattr(model, 'module'):
            model.module.fc = nn.Sequential(nn.Linear(model.module.fc.weight.size(1), args.Hidden, bias=bias_fe),
                                            nn.ReLU(),
        ...
        if args.recurbase:
            ...
            R = IL_align(train_loader, model, args, R,phase, 1)
        else:
            R = cls_align(train_loader, model, args)
        ...
        for phase in range(args.phase):
            ...
            if hasattr(model, 'module'):
                W = model.module.fc[-1].weight
                W = torch.cat([W, torch.zeros(nc_each, args.Hidden).cuda(args.gpu)], dim=0)
                model.module.fc[-1] = nn.Linear(args.Hidden, args.num_classes, bias=False)
                model.module.fc[-1].weight = torch.nn.parameter.Parameter(W.float())
            else:
                W = model.fc[-1].weight
                W = torch.cat([W, torch.zeros(nc_each, args.Hidden).cuda(args.gpu)], dim=0)
                model.fc[-1] = nn.Linear(args.Hidden, args.num_classes, bias=False)
                model.fc[-1].weight = torch.nn.parameter.Parameter(W.float())
            R = IL_align(IL_dataset_train[phase], model, args, R,phase, args.repeat)
            ...
            '''with open('/content/drive/MyDrive/ACIL_BASELINE-20240219T093230Z-001/Base_acc.csv', 'a', newline='') as csvfile:
        ...
        with open(args.dataset + args.arch + '_' + str(args.phase) + '.csv', mode='a+', encoding="ISO-8859-1", newline='') as file:
            ...
            wr = csv.writer(file)
            wr.writerow(data)
        print('written it to a csv file named {}.'.format(args.dataset + args.arch + '.csv'))
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        '''with open('/content/drive/MyDrive/ACIL_BASELINE-20240219T093230Z-001/Base_training.csv', 'a', newline='') as csvfile:
          writer = csv.writer(csvfile)
...
def IL_align(train_loader, wrapped_model, args, R,phase, repeat):
    ...
    with torch.no_grad():
        for epoch in range(repeat):
            ...
            for i, (images, target) in pbar:
                ...
                #R = torch.inverse(torch.inverse(R) +0.01*torch.eye(new_activation.shape[1]).to(new_activation))
                ...
                W = W + R @ new_activation.t() @ (label_onehot - new_activation @ W) #+ 0.01*R @ W
    ...
    torch.save(model.fc[0].weight,f"ACIL_CUB_new_weights_linear_{phase}_new.pth")
    torch.save(model.fc[2].weight,f"ACIL_CUB_new_weights{phase}_new.pth")
    return R
...
def validate(val_loader, model, criterion, args, print=True):
    ...
    with torch.no_grad():
        ...
        for i, (images, target) in enumerate(val_loader):
            ...
            '''with open('/content/drive/MyDrive/ACIL_BASELINE-20240219T093230Z-001/Base_Validation.csv', 'a', newline='') as csvfile:
              writer = csv.writer(csvfile)
